...
if ($query->have_posts()) : while ($query->have_posts()) : $query->the_post();
	...
	if(isset($_GET['post'])) {			
...

		{
			...
			$post_location = get_post_meta($post->ID, 'post_location', true);						$post_state = get_post_meta($post->ID, 'post_state', true);			$post_city = get_post_meta($post->ID, 'post_city', true);
			...
			$post_video = get_post_meta($post->ID, 'post_video', true);						$featuredIMG = get_post_meta($post->ID, 'featured_img', true);						$itemCondition = get_post_meta($post->ID, 'item-condition', true);						$classiera_ads_type = get_post_meta($post->ID, 'classiera_ads_type', true);						$classiera_post_type = get_post_meta($post->ID, 'classiera_post_type', true);			$pay_per_post_product_id = get_post_meta($post->ID, 'pay_per_post_product_id', true);			$days_to_expire = get_post_meta($post->ID, 'days_to_expire', true);			
			$featured_post = "0";
...
if(isset($_POST['submitted']) && isset($_POST['post_nonce_field']) && wp_verify_nonce($_POST['post_nonce_field'], 'post_nonce')) {	
	if(trim($_POST['postTitle']) === '') {				$postTitleError =  esc_html__( 'Please select a title.', 'classiera' );
		$hasError = true;
	} else {
		$postTitle = trim($_POST['postTitle']);
	}
	if(empty($_POST['cat'])){				$catError =  esc_html__( 'Please select a category.', 'classiera' );
		$hasError = true;
	} 		
	if($hasError != true) {		//$classieraPostType = $_POST['classiera_post_type'];
		if(is_super_admin() ){
			$postStatus = 'publish';
			...
			}			if($classieraPostType == 'payperpost'){				$postStatus = 'pending';			}
		}				//Check Category//		
		if(trim($_POST['cat']) === '-1') {						
			$catError =  esc_html__( 'Please select a category.', 'classiera' );			$hasError = true;		} 		
		$mCatID = $_POST['cat'];		$classieraGetCats = classiera_get_cats_on_edit($mCatID);				
		$categoriesID = get_ancestors($mCatID, 'category', 'taxonomy');		if(!empty($categoriesID)){			
			$cats = "";			foreach ($categoriesID as $id) {				$cats .= $id.', ';							}			
		$catsString = $cats.$mCatID;						
		$catsArray = explode(",", $catsString);		}else{			$catsArray = array($mCatID);		}				$post_main_cat = $classieraGetCats['post_main_cat'];		$post_child_cat = $classieraGetCats['post_child_cat'];		$post_inner_cat = $classieraGetCats['post_inner_cat'];		
		...
		$post_information = array(
			...
			'post-type' => 'post',			'post_date' => $post_date,
			'post_category' => $catsArray,
	        'tags_input'    => explode(',', $_POST['post_tags']),			'tax_input' => array(				'location' => $_POST['post_location'],			),
	        'comment_status' => 'open',
	        'ping_status' => 'open',			'post_author' => $_POST['postAuthor'],
			'post_status' => $postStatus
		);		
		$post_id = wp_insert_post($post_information);		$googleLat = $_POST['latitude'];
		...
		global $redux_demo; 
		if(isset($redux_demo['free_price_text'])):
		...
		if(isset($_POST['post_location'])){			
			...
			$allCountry = get_posts( array( 'include' => $postLo, 'post_type' => 'countries', 'posts_per_page' => -1, 'suppress_filters' => 0, 'orderby'=>'post__in' ) );			foreach( $allCountry as $country_post ){				$postCounty = $country_post->post_title;			}		}		if(isset($_POST['post_state'])){			$poststate = $_POST['post_state'];		}		if(isset($_POST['post_state'])){			$postCity = $_POST['post_city'];		}		
			if(isset($_POST['classiera_CF_Front_end'])){			
				$classiera_CF_Front_end = $_POST['classiera_CF_Front_end'];		
			}else
			{
				$classiera_CF_Front_end='';
			}		
		...
		update_post_meta($post_id, 'classiera_ads_type', $_POST['classiera_ads_type']);	
		endif;			
		update_post_meta($post_id, 'post_perent_cat', $post_main_cat);		
		update_post_meta($post_id, 'post_child_cat', $post_child_cat);						
		update_post_meta($post_id, 'post_inner_cat', $post_inner_cat);				
		update_post_meta($post_id, 'post_currency_tag', $postMultiTag);
		update_post_meta($post_id, 'post_price', $post_price_content);				
		update_post_meta($post_id, 'post_old_price', $old_price_content);		
		if(isset($_POST['post_phone'])){		
			update_post_meta($post_id, 'post_phone', $_POST['post_phone']);		
		}
		...
		update_post_meta($post_id, 'post_video', $_POST['video']);				
		update_post_meta($post_id, 'featured_img', $_POST['featured-image']);				
		if(isset($_POST['item-condition'])){			
			update_post_meta($post_id, 'item-condition', $itemCondition);		
		}
		// update_post_meta($post_id, 'classiera_post_type', $_POST['classiera_post_type'], $allowed);		update_post_meta($post_id, 'pay_per_post_product_id', $_POST['pay_per_post_product_id'], $allowed);		update_post_meta($post_id, 'days_to_expire', $_POST['days_to_expire'], $allowed);
		$permalink = get_permalink( $post_id );
		//Rest featured image from old images//		if(isset($_POST['classiera_featured_img'])){			$imageID = $_POST['classiera_featured_img'];			set_post_thumbnail( $post_id, $imageID );		}
		//If Its posting featured image//		if(trim($_POST['classiera_post_type']) != 'classiera_regular'){			if($_POST['classiera_post_type'] == 'payperpost'){				//Do Nothing on Pay Per Post//			}elseif($_POST['classiera_post_type'] == 'classiera_regular_with_plan'){				//Regular Ads Posting with Plans//				$classieraPlanID = trim($_POST['regular_plan_id']);				global $wpdb;				$current_user = wp_get_current_user();				$userID = $current_user->ID;				$result = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}classiera_plans WHERE id = $classieraPlanID" );				if($result){					$tablename = $wpdb->prefix . 'classiera_plans';					$newRegularUsed = $info->regular_used +1;					$update_data = array('regular_used' => $newRegularUsed);					$where = array('id' => $classieraPlanID);					$update_format = array('%s');					$wpdb->update($tablename, $update_data, $where, $update_format);				}			}else{				//Featured Post with Plan Ads//				$featurePlanID = trim($_POST['classiera_post_type']);				global $wpdb;				$current_user = wp_get_current_user();				$userID = $current_user->ID;				$result = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}classiera_plans WHERE id = $featurePlanID" );				if ($result){					$featuredADS = 0;					$tablename = $wpdb->prefix . 'classiera_plans';					foreach ( $result as $info ){						$totalAds = $info->ads;						if (is_numeric($totalAds)){							$totalAds = $info->ads;							$usedAds = $info->used;							$infoDays = $info->days;						}														if($totalAds == 'unlimited'){							$availableADS = 'unlimited';						}else{							$availableADS = $totalAds-$usedAds;						}														if($usedAds < $totalAds && $availableADS != "0" || $totalAds == 'unlimited'){							global $wpdb;							$newUsed = $info->used +1;							$update_data = array('used' => $newUsed);							$where = array('id' => $featurePlanID);							$update_format = array('%s');							$wpdb->update($tablename, $update_data, $where, $update_format);							update_post_meta($post_id, 'post_price_plan_id', $featurePlanID );							$dateActivation = date('m/d/Y H:i:s');							update_post_meta($post_id, 'post_price_plan_activation_date', $dateActivation );																$daysToExpire = $infoDays;							$dateExpiration_Normal = date("m/d/Y H:i:s", strtotime("+ ".$daysToExpire." days"));							update_post_meta($post_id, 'post_price_plan_expiration_date_normal', $dateExpiration_Normal );							$dateExpiration = strtotime(date("m/d/Y H:i:s", strtotime("+ ".$daysToExpire." days")));							update_post_meta($post_id, 'post_price_plan_expiration_date', $dateExpiration );							update_post_meta($post_id, 'featured_post', "1" );						}					}				}			}		}
		if ( $_FILES ) {			$count = 0;
			...
			foreach ($files['name'] as $key => $value) {
				if ($files['name'][$key]) {
					...
					$_FILES = array("upload_attachment" => $file);										foreach ($_FILES as $file => $array){														$featuredimg = $_POST['classiera_featured_img'];						if($count == $featuredimg){							$attachment_id = classiera_insert_attachment($file,$post_id);							set_post_thumbnail( $post_id, $attachment_id );						}else{							$attachment_id = classiera_insert_attachment($file,$post_id);						}														$count++;					}
				}
			}
		}
				...
				<div class="col-lg-9 col-md-8 user-content-height">				
					<div class="submit-post section-bg-white classiera_edit__post">					
						<form class="form-horizontal" action="" role="form" id="primaryPostForm" method="POST" data-toggle="validator" enctype="multipart/form-data">						
							<h4 class="text-uppercase border-bottom"><?php esc_html_e('EDIT YOUR AD', 'classiera') ?></h4>						
							...
							<div class="form-main-section classiera-post-cat">							
								...
								<select id="cat" class="postform" name="cat">									
									...
									$args = array(											'hierarchical' => '0',											'hide_empty' => '0'										);										$categories = get_categories($args);										
									foreach ($categories as $cat){											
										if($cat->category_parent == 0){												
											$catID = $cat->cat_ID;												
											?>												
											<option <?php if($currCatID == $catID){echo "selected";} ?> value="<?php echo esc_attr($cat->cat_ID); ?>">													
												<?php echo esc_attr($cat->cat_name); ?>												</option>												
												<?php												
												$args2 = array(													'hide_empty' => '0',													'parent' => $catID												);												$categories = get_categories($args2);												
												foreach($categories as $cat){													
													$catSubID = $cat->cat_ID;
													?>													
													<option <?php if($currCatID == $cat->cat_ID){echo "selected";} ?> value="<?php echo esc_attr($cat->cat_ID); ?>">-- 														<?php 
													echo esc_attr($cat->cat_name); ?>													</option>													
													<?php													
													$args3 = array(														'hide_empty' => '0',														'parent' => $catSubID													);													$categories = get_categories($args3);													
													foreach($categories as $cat){														
														?>														
														<option <?php if($currCatID == $cat->cat_ID){echo "selected";} ?> value="<?php echo esc_attr($cat->cat_ID); ?>">--- 															<?php 
														echo esc_attr($cat->cat_name); ?>														</option>														
								...
								</div><!--classiera-post-cat-->						<!--AdsDetails-->						
													...
													<div class="form-inline row">                                        
														...
														if($adpostCondition == 1){							
															...
															<div class="form-group">                                
																...
																<div class="col-sm-9">                                    
								<div class="radio">                                        
								<input id="new" type="radio" name="item-condition" value="<?php esc_html_e('new', 'classiera') ?>" name="item-condition" <?php if($itemCondition == 'new'){
									echo "checked";}?>>                                        
									<label for="new"><?php esc_html_e('Brand New', 'classiera') ?></label>                                        
									<input id="used" type="radio" name="item-condition" value="<?php esc_html_e('used', 'classiera') ?>" name="item-condition" <?php if($itemCondition == 'used'){
										echo "checked";}?>>                                        
							...
							<div class="form-main-section extra-fields wrap-content" id="cat-<?php echo esc_attr($user_id); ?>" 
								...
								for($i = 0; $i < (count($wpcrown_category_custom_field_option)); $i++){ 									
									if($wpcrown_category_custom_field_type[$i][1] == 'text'){										?>										
										<div class="form-group" id="cat-<?php echo esc_attr($user_id); ?>">											
											<label class="col-sm-3 text-left flip"><?php echo esc_attr($wpcrown_category_custom_field_option[$i][0]); ?>: <span>*</span></label>											
											<div class="col-sm-6">												
												<input type="hidden" class="custom_field" id="custom_field[<?php echo esc_attr($i); ?>][0]" name="<?php echo esc_attr($user_id); ?>custom_field[<?php echo esc_attr($i); ?>][0]" value="<?php echo esc_attr($wpcrown_category_custom_field_option[$i][0]) ?>" size="12">																						
												<input type="hidden" class="custom_field" id="custom_field[<?php echo esc_attr($i); ?>][2]" name="<?php echo esc_attr($user_id); ?>custom_field[<?php echo esc_attr($i); ?>][2]" value="<?php echo esc_attr($wpcrown_category_custom_field_type[$i][1]) ?>" size="12">																								
												<input type="text" placeholder="<?php if (!empty($wpcrown_category_custom_field_option[$i][0])) echo esc_attr($wpcrown_category_custom_field_option[$i][0]); ?>" class="form-control form-control-md" id="custom_field[<?php echo esc_attr($i); ?>][1]" name="<?php echo esc_attr($user_id); ?>custom_field[<?php echo esc_attr($i); ?>][1]" value="<?php if($currCatID == $user_id) { echo esc_attr($wpcrown_custom_fields[$i][1]); } ?>" size="12">											
											</div>										
										</div>										
										<?php									
									}								}							
									...
									for ($i = 0; $i < (count($wpcrown_category_custom_field_option)); $i++) {								
										if($wpcrown_category_custom_field_type[$i][1] == 'dropdown'){							?>								
											<div class="form-group" id="cat-<?php echo esc_attr($user_id); ?>">									<label class="col-sm-3 text-left flip"><?php echo esc_attr($wpcrown_category_custom_field_option[$i][0]); ?>: <span>*</span></label>									<div class="col-sm-6">										
												<div class="inner-addon right-addon">											
													<i class="form-icon right-form-icon fa fa-angle-down"></i>											
													<input type="hidden" class="custom_field" id="custom_field[<?php echo esc_attr($i); ?>][0]" name="<?php echo esc_attr($user_id); ?>custom_field[<?php echo esc_attr($i); ?>][0]" value="<?php echo esc_attr($wpcrown_category_custom_field_option[$i][0]) ?>" size="12">											
													<input type="hidden" class="custom_field" id="custom_field[<?php echo esc_attr($i); ?>][2]" name="<?php echo esc_attr($user_id); ?>custom_field[<?php echo esc_attr($i); ?>][2]" value="<?php echo esc_attr($wpcrown_category_custom_field_type[$i][1]); ?>" size="12">																						
													<select class="form-control form-control-md" id="custom_field[<?php echo esc_attr($i); ?>][1]" name="<?php echo esc_attr($user_id); ?>custom_field[<?php echo esc_attr($i); ?>][1]">												
														<?php												echo '<option>'.$wpcrown_category_custom_field_option[$i][0].'</option>';												
														$options = $wpcrown_category_custom_field_type[$i][2];												
														$optionsarray = explode(',',$options);												foreach($optionsarray as $option){													if($wpcrown_custom_fields[$i][1] == $option){														$selected = 'selected';													}													
														else{														
															$selected = '';													
														}													
										...
										{								 
											if($wpcrown_category_custom_field_type[$i][1] == 'checkbox'){									
												if($wpcrown_custom_fields[$i][1] == 'on'){										
													...
													$checked = '';									} 							?>								
													<div class="form-group" id="cat-<?php echo esc_attr($user_id); ?>">									<p class="featurehide featurehide<?php echo esc_attr($i); ?>"><?php esc_html_e('Select Features', 'classiera') ?></p>									<div class="col-sm-6">										
														<div class="inner-addon right-addon">											
															<i class="form-icon right-form-icon fa fa-angle-down"></i>											
															<input type="hidden" class="custom_field" id="custom_field[<?php echo esc_attr($i); ?>][0]" name="<?php echo esc_attr($user_id); ?>custom_field[<?php echo esc_attr($i); ?>][0]" value="<?php echo esc_attr($wpcrown_category_custom_field_option[$i][0]); ?>" size="12">																						
						<input type="hidden" class="custom_field" id="custom_field[<?php echo esc_attr($i); ?>][2]" name="<?php echo esc_attr($user_id); ?>custom_field[<?php echo esc_attr($i); ?>][2]" value="<?php echo esc_attr($wpcrown_category_custom_field_type[$i][1]); ?>" size="12">																																	
						<div class="checkbox">												
							<input type="checkbox" <?php echo esc_attr($checked); ?> class="custom_field custom_field_visible input-textarea newcehckbox" id="<?php echo esc_attr($user_id); ?>custom_field[<?php echo esc_attr($i); ?>][1]" name="<?php echo esc_attr($user_id); ?>custom_field[<?php echo esc_attr($i); ?>][1]">												
							<label for="<?php echo esc_attr($user_id); ?>custom_field[<?php echo esc_attr($i); ?>][1]" class="newcehcklabel"><?php echo esc_attr($wpcrown_category_custom_field_option[$i][0]); ?></label>											</div>										
						</div>									
					</div>								
				</div>							
				<?php 								
